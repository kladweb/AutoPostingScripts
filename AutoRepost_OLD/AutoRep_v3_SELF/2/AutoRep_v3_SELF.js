...
(function () {
  ...
  var action265 = function (nextAction, numberGroup) {
    ...
    setTimeout(() => {
      ...
      if (linkCurGroup) {
        linkCurGroup.click();
        action267checkNewPosts(nextAction, numberGroup);
        loopAct = 0;
      } else {
        loopAct++;
        console.log(`action265: не найдена ссылка группы ${numberGroup}. Повторная попытка поиска...`);
        if (loopAct > 5) {
          console.log('Превышено количество итераций, пробуем исправить...')
          setTimeout(() => {
            var myAcc = document.querySelector(`a[aria-label="Василий Тис"]`);
            myAcc.click();
            // kolIter--;
            loopAct = 0;
            action250(action260);
  ...
  var action267checkNewPosts = function (nextAction, numberGroup) {
    console.log('action267');
    setTimeout(() => {
      ...
      if (feedWs[0]) {
        ...
        var identD = feedWs[t].querySelector('.feed-user-avatar');
        if (identD) {
          var identImg = identD.querySelector(`img.feed-avatar-img`);
          if (identImg) {
            var checkIdAcc = parseInt(identImg.getAttribute('alt'));
